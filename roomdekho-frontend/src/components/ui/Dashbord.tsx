...
const Dashboard = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100 font-sans overflow-hidden">
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-20 bg-gray-800 text-white flex flex-col transform transition-transform duration-300 ease-in-out 
        ...
        <header className="bg-white shadow-md p-4 flex items-center justify-between z-10 sticky top-0">
          <div className="flex items-center">
            <button
              ...
              className="text-gray-600 mr-4 p-2 rounded-full hover:bg-gray-200 md:hidden"
            >
          ...
          <div className="flex items-center space-x-4 md:space-x-6">
            <div className="relative hidden md:block">
              <input
                type="text"
                placeholder="Search..."
                className="pl-10 pr-4 py-2 w-40 sm:w-48 rounded-full bg-gray-100 focus:outline-none focus:ring-2 focus:ring-yellow-500"
              />
              <FontAwesomeIcon
                icon={faSearch}
            ...
            <FontAwesomeIcon
              icon={faBell}
              className="text-xl sm:text-2xl text-gray-600 cursor-pointer"
            />
            <img
  ...
  <div className="space-y-6 sm:space-y-8">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
      <div className="lg:col-span-2 bg-white p-4 sm:p-6 rounded-lg shadow-md">
        <h3 className="text-lg sm:text-xl font-semibold mb-4">User Growth</h3>
      ...
      <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
        <h3 className="text-lg sm:text-xl font-semibold mb-4">Property Status</h3>
        <ResponsiveContainer width="100%" height={250}>
          <PieChart>
            <Pie
    ...
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
      <h3 className="text-lg sm:text-xl font-semibold mb-4">Monthly Revenue (₹)</h3>
      <ResponsiveContainer width="100%" height={250}>
...
const StatCard = ({ title, value }: { title: string; value: string }) => (
  <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md hover:shadow-xl hover:-translate-y-1 transition-all">
    <h3 className="text-gray-500 text-sm sm:text-base">{title}</h3>
    <p className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-800 mt-2">{value}</p>
  </div>
);
...
const PropertiesManagement = () => (
  <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-6">
      <h2 className="text-xl sm:text-2xl font-semibold">Manage Properties</h2>
      {/* Future filter/search can go here */}
    </div>
    <div className="overflow-x-auto">
      <table className="min-w-full bg-white text-sm sm:text-base">
        <thead className="bg-gray-200">
          <tr>
            <th className="py-2 px-3 sm:py-3 sm:px-4 text-left">Owner</th>
            <th className="py-2 px-3 sm:py-3 sm:px-4 text-left">Property Type</th>
            <th className="py-2 px-3 sm:py-3 sm:px-4 text-left">Location</th>
            <th className="py-2 px-3 sm:py-3 sm:px-4 text-left">Status</th>
        ...
        <tbody>
          {propertiesData.map((prop) => (
            <tr key={prop.id} className="border-b hover:bg-gray-50">
              <td className="py-2 px-3 sm:py-3 sm:px-4">{prop.owner}</td>
              <td className="py-2 px-3 sm:py-3 sm:px-4">{prop.type}</td>
              <td className="py-2 px-3 sm:py-3 sm:px-4">{prop.location}</td>
              ...
              <td className="py-2 px-3 sm:py-3 sm:px-4 text-center space-x-2 sm:space-x-3">
